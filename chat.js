...
// --- NOTA PERMANENTE DE ARQUITECTURA ---
// NO USAR SDK DE GOOGLE PARA GEMINI: El SDK a menudo fuerza la versión 'v1beta' que da errores 404 en modelos estables.
// USAR SIEMPRE FETCH DIRECTO A LA VERSION 'v1' PARA MÁXIMA ESTABILIDAD.
...
async function processChat(req) {
    ...
    // --- CADENA DE MANDOS (ESTABILIZADA 2026) ---

    // 1. GEMINI (LÍDER - VIA FETCH DIRECTO A v1 PARA EVITAR 404 DE v1beta)
    if (process.env.GEMINI_API_KEY) {
        try {
            console.log("🚀 Liderando con Gemini 1.5 Flash (API v1)...");
            const timeoutMs = isBriefing ? 8000 : 7000;

            // Construimos el cuerpo para la API REST de Google
            const requestBody = {
                contents: [
    ...
    if (process.env.ANTHROPIC_API_KEY) {
        ...
        for (const modelName of models) {
            try {
                ...
                // Timeouts extendidos para evitar fallos en procesos de fondo
                ...
                // Si el modelo da 404 de verdad, seguimos al siguiente. Si es timeout, paramos para no agotar el tiempo de Vercel.
...
// Formateador para la API REST de Gemini
...
// Mantener compatibilidad con el SDK (aunque ya no lo usemos como primario)
